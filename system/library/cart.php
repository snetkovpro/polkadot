	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				//dev 
				$product_special_query_dev = $this->db->query("SELECT price FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$cart['product_id'] . "' AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) ORDER BY priority DESC, price ASC LIMIT 1");

				if ($product_special_query_dev->num_rows) {

					$price = $product_special_query_dev->row['price'];
				}

				$prices_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_prices WHERE product_id = '" . (int)$cart['product_id'] . "'");
				$pack_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_pack WHERE product_id = '" . (int)$cart['product_id'] . "'");

				foreach ($prices_query->row as $key => $value) {
					$prices[$key] = (float)$value;
				}

				foreach ($pack_query->row as $key => $value) {
					$pack[$key] = (float)$value;
				}

				// var_dump($this->customer->getId());

				if ($this->customer->getId()) {
					$price = ($prices['wholesale_discount'] != 0) ? $prices['wholesale_discount'] : $prices['wholesale'];
					if (isset($pack['pack']) && $pack['pack'] > 1) {
						$packprice = $price * $pack['pack'];
					} else {
						$packprice = $price;
					}
				} else {
					$price = ($prices['retail_discount'] != 0) ? $prices['retail_discount'] : $prices['retail'];
					if (isset($pack['retail']) && $pack['retail'] > 1) {
						$packprice = $price * $pack['retail'];
					} else {
						$packprice = $price;
					}
				}
