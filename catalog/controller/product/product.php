...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if (isset($_SERVER['HTTP_REFERER'])) {
			...
			$data['test'] = $link = $_SERVER['HTTP_REFERER'];
			$data['test2'] = $this->config->get('config_url');


			while ($link !== $this->config->get('config_url')) {
				...
				$link = substr_replace($link, '/', strrpos($link, '/', -2));
		...
		$minQuantity = $this->model_catalog_product->getMinQuantity($product_id);
		if ($this->customer->isLogged()) {
		    $customer_group_id = $this->customer->getGroupId();
		    
		    $minimum = $minQuantity['min_quantity']; //#roma
		} else {
		    $minimum = $minQuantity['retail'];
		    $customer_group_id = '';
		}
		...
		if ($product_info) {
			...
			if ($this->customer->isLogged()) {
				$pack = $this->model_catalog_product->getPack($product_id)['pack'];
			} else {
				$pack = $this->model_catalog_product->getPack($product_id)['retail']; 
			}
			...
			$prices = $this->model_catalog_product->getPrices($product_info['product_id']);
...
//prices & discounts
			...
			$data['retail'] = $this->currency->format($prices['retail']);
			$data['wholesale'] = $this->currency->format($prices['wholesale']);

			if ($this->customer->isLogged()) {
				
				$data['special'] = $this->currency->format($prices['wholesale_discount']);
				$disc = (empty($prices['wholesale_discount']) || $prices['wholesale_discount'] == 0) ? 0 : (100-$prices['wholesale_discount']*100/$prices['wholesale']);
			} else {
				
				$data['special'] = $this->currency->format($prices['retail_discount']);
				$disc = (empty($prices['retail_discount']) || $prices['retail_discount'] == 0) ? 0 : (100-$prices['retail_discount']*100/$prices['retail']);
			}
