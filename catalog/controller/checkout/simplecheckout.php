...
class ControllerCheckoutSimpleCheckout extends SimpleController {
    ...
    public function index($args = null) {
        ...
        // So don't touch this my code
        
        $this->_templateData['test'] = 'This is test';
        $this->load->model('catalog/product');
        $product_data = $this->cart->getProducts();
        
         $this->_templateData['customer_group'] = $this->cart->customer->getGroupId(); 
        $cold_products = array('18', '113', '114', '115', '69');
        $priznak = false;
         
        foreach ($product_data as $product) {
            foreach ($this->model_catalog_product->getCategories($product['product_id']) as $categories) {
                $this->_templateData['categories'][] = $categories['category_id'];
                if (!$priznak){
                $priznak = in_array($categories['category_id'], $cold_products);
                }
            }
            
        }
        
        $this->_templateData['disclaimer'] = $this->config->get('config_disclaimer') && $priznak;
        $this->_templateData['priznak'] = $priznak;
