...
class ControllerCheckoutCheckout extends Controller {
	public function index() {
		...
		$customer_id = ($this->customer->isLogged()) ? $this->customer->isLogged() : 0;

		foreach ($products as $product) {
			$product_total = 0;
            	if ($this->customer->isLogged()) {
		            $customer_group_id = $this->customer->getGroupId();
		            $minimum_customer = $this->model_catalog_product->getMinQuantity($product['product_id'], $customer_group_id) ? implode($this->model_catalog_product->getMinQuantity($product['product_id'], $customer_group_id)) : 1;
		        } else {
		            $minimum_customer = 1;
		        }
			...
			if ($minimum_customer > $product_total) {
				$this->response->redirect($this->url->link('checkout/cart'));
			}
		}
