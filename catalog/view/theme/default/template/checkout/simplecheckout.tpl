...
<p class="disclaimer">Внимание! В зимнее время мы рекомендуем отправку красок и спреев теплом ТК «ЖелДорЭкспедиция». При отправке другой ТК, мы не несем ответственность за качество данных позиций. </p>
<?php } else { ?>
<p class="disclaimer">ВНИМАНИЕ! Не рекомендуется отправка Почтой РФ в зимнее время. Оформляя заказ в это время, Вы берете всю ответственность на себя. </p>
<?php } ?>
<?php } ?>
<p class="hidden"><?php var_dump($categories, $customer_group, $disclaimer, $priznak, $simple_header); ?> </p>
