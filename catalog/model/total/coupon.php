...
class ModelTotalCoupon extends Model {
	public function getCoupon($code) {
		...
		$coupon_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon` WHERE code = '" . $this->db->escape($code) . "' AND ((date_start = '0000-00-00' OR date_start < NOW() OR date_start = CURRENT_DATE()) AND (date_end = '0000-00-00' OR date_end > NOW() OR date_end = CURRENT_DATE())) AND status = '1'");
	...
	public function getTotal(&$total_data, &$total, &$taxes) {
		if (isset($this->session->data['coupon'])) {
			...
			if ($coupon_info) {
				...
				// var_dump($sub_total, $amount);
				// If discount greater than total
				if ($amount > $sub_total) {
				//	$discount_total = isset($this->session->data['shipping_method']) ? $total + $this->session->data['shipping_method']['cost'] : $total;
					if (isset($this->session->data['shipping_method'])) {
						$discount_total = (($discount_total + $this->session->data['shipping_method']['cost']) > $amount) ? $amount : ($discount_total + $this->session->data['shipping_method']['cost']);
					} else {
