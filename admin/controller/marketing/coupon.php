...
class ControllerMarketingCoupon extends Controller {
	...
	public function coupon_mail($data) {

		

		if ($data['sum']) {
			$html = '<html><head></head><body><p>' . $data['customer'] . ', добрый день!</p><br/>
			<p>На товары сложеные в вашей корзине действует дополнительная скидка ' . $data['discount'] . '% в период с ' . $data['date_start'] . ' по ' . $data['date_end'] . '. <br/>Скидка суммируется с программой лояльности.</p>
			<p>Для того чтобы воспользоваться скидкой, при оформлении заказа необходимо ввести купон – ' . $data['code'] . '.</p>
			<br/><p>С уважением,<br/>polkadot.su</p></body></html>';
		} else {

			$html = '<html><head></head><body><p>' . $data['customer'] . ', добрый день!</p><br/>
			<p>На товары сложеные в вашей корзине действует дополнительная скидка ' . $data['discount'] . '% в период с ' . $data['date_start'] . ' по ' . $data['date_end'] . '. <br/>Скидка не суммируется с программой лояльности.</p>
			<p>Для того чтобы воспользоваться скидкой, при оформлении заказа необходимо ввести купон – ' . $data['code'] . '.</p>
			<br/><p>С уважением,<br/>polkadot.su</p></body></html>';
		...
		$mail->setSubject(html_entity_decode('Скидка на товары в корзине!', ENT_QUOTES, 'UTF-8'));
		$mail->setHtml($html);
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//dev
			if (isset($this->request->post['letter'])) {
				$coupon_data = array();
				$coupon_data['name'] = $this->request->post['name'];
				$coupon_data['discount'] = $this->request->post['discount'];
				$coupon_data['email'] = $this->request->post['email'];
				$coupon_data['date_start'] = date($this->language->get('date_format_short'), strtotime($this->request->post['date_start']));
				$coupon_data['date_end'] = date($this->language->get('date_format_short'), strtotime($this->request->post['date_end']));;
				$coupon_data['code'] = $this->request->post['code'];
				$coupon_data['sum'] = $this->request->post['sum'];
				$coupon_data['logged'] = $this->request->post['logged'];
				$coupon_data['customer'] = $this->request->post['customer'];

				$this->coupon_mail($coupon_data);
			}
	...
	protected function getForm() {
		...
		if (isset($this->request->post['date_end'])) {
			...
			$data['date_end'] = date('Y-m-d', strtotime("+1 month", time()));		
		}
