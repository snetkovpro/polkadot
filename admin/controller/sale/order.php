	...
	public function info() {
		...
		if ($order_info) {
			...
			foreach ($products as $product) {
				...
				$category = $this->model_catalog_product->getProductCategories($product['product_id'])[0];
				$parent_id = 1;
				// $products = $this->model_catalog_category->getCategory($category);
			...
			// roman sort
			    ...
			    */
			    // $data['sortarray'] = $data['products'];
			    // array_multisort($sortarray['category'], SORT_ASC, $data['products']);
			     usort($data['products'], array($this, 'usort_callback'));
			...
			foreach ($totals as $total) {
				...
				$coup = strrpos($total['title'], 'Купон');

				if ($coup !== false) {
					preg_match('/\((.+)\)/', $total['title'], $coupon);
					$coupon_id = $coupon[1] ? $this->model_marketing_coupon->getCouponByCode($coupon[1])['coupon_id'] : 0;
					$data['coupon_product'] = $this->model_marketing_coupon->getCouponProducts($coupon_id);
					$data['discount'] = intval($this->model_marketing_coupon->getCouponByCode($coupon[1])['discount']);
				} 
			}
