<?php
class ControllerSaleActualCart extends Controller {
	private $error = array();
	...
	protected function getList() {
		...
		$myquery = $this->db->query("SELECT * FROM " . DB_PREFIX . "cart");


		
		foreach ($myquery->rows as $mycart) {

			$customer = $this->model_customer_customer->getCustomer($mycart['customer_id']) ? $this->model_customer_customer->getCustomer($mycart['customer_id'])['firstname'] . ' ' . $this->model_customer_customer->getCustomer($mycart['customer_id'])['lastname'] : 'noname';
					...
					$actual_cart[] = array (
						...
						'view'			=>	$this->url->link('sale/actual_cart/getCart', 'token=' . $this->session->data['token'] . '&customer_id=' . $mycart['customer_id'], 'SSL'),
						'customer'		=>	$customer,
						'date'			=>	$mycart['date_added']
			);


					$actual_cart = unique_multidim_array($actual_cart, 'customer');
		...
		foreach ($actual_cart as $expired) {
			if ($expired['date'] > '2017-11-01 00:00:00') {
				$data['actual_cart'][] = $expired;
			}


		}

		krsort($data['actual_cart']);
		...
		$data['column_customer_group_id'] = $this->language->get('text_customer_group');
		...
		$data['sort_customer_group_id'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=customer_group_id' . $url, 'SSL');
		...
		$this->response->setOutput($this->load->view('sale/cart_list.tpl', $data));
	}
	...
	public function getCart() {
		...
		$myquery = $this->db->query("SELECT * FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int)$customer_id . "'");
		$data['customer'] = $this->model_customer_customer->getCustomer($customer_id)['firstname'] . ' ' . $this->model_customer_customer->getCustomer($customer_id)['lastname'];
		$data['email'] = $this->model_customer_customer->getCustomer($customer_id)['email'] ? $this->model_customer_customer->getCustomer($customer_id)['email'] : '';
		$data['product'] = array();
		$i = 0;

		foreach ($myquery->rows as $product) {
			$data['product'][$i]['product_id'] = $product['product_id'];
			$data['product'][$i]['name'] = $this->model_catalog_product->getProduct($product['product_id'])['name'];
			$data['product'][$i]['art'] = $this->model_catalog_product->getProduct($product['product_id'])['sku'];
			$data['product'][$i]['quantity'] = $product['quantity'];
			$data['product'][$i]['price'] = $this->currency->format($this->model_catalog_product->getProduct($product['product_id'])['price']);
			$data['product'][$i]['sum'] = intval($this->model_catalog_product->getProduct($product['product_id'])['price']) * $product['quantity'];
			...
			$data['pr_coupon'] = $this->url->link('marketing/coupon/add', 'token=' . $this->session->data['token'], 'SSL');
			$data['temp'] = $myquery;
			$data['header'] = $this->load->controller('common/header');
			$data['column_left'] = $this->load->controller('common/column_left');
			$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('sale/cart_view.tpl', $data));
		...
		if (!empty($order_info)) {
			...
			foreach ($products as $product) {
				$data['order_products'][] = array(
					...
					'name'       => $product['name'],
					'model'      => $product['quantity'], $product['model'],
	...
	public function info() {
		...
		if ($order_info) {
			...
			foreach ($products as $product) {
				...
				$productf = $this->model_catalog_product->getProduct($product['product_id']);

				$data['products'][] = array(
					'order_product_id' => $product['order_product_id'],
					'product_id'       => $product['product_id'],
					'name'    	 	   => $product['name'],
					'model'    		   => $productf['sku'], //$product['model'],
					'option'   		   => $option_data,
					'quantity'		   => $product['quantity'],
			...
			// roman sort
			    $data['old_products'] = $data['products'];
			    $sortarray = array();
			    ...
			    array_multisort($sortarray['model'], SORT_ASC, $data['products']);
			...
			//added by Roman
			$data['andrew_fields'] = array("ИНН", "КПП", "ОГРН", "Доп. упаковка", "Отправка теплом", "Стоимость груза, указываемая в ТК", "Доставка до адреса", "Страховка",
			"Наименование юрлица", "Транспортная компания");
			$data['address_fields'] = array("Регион/Область", "Почтовый индекс", "Город", "Улица, номер дома и квартиры");
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
			$order_info = $this->model_sale_order->getOrder($order_id);
				...
				$data['name'] = $order_info['shipping_firstname'];
